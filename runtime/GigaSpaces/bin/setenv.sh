...
if [ -z "${JAVA_OPTIONS}" ]; then
  case $JAVA_VENDOR in
  BEA)
    JAVA_OPTIONS=" -server -showversion -Xmx512m"
#-Xgc:gencon"
  ;;
  HP)
    JAVA_OPTIONS=" -server -showversion -Xmx512m"
  ;;
  IBM)
    JAVA_OPTIONS=" -showversion -Xmx512m"
  ;;
  Sun)
    JAVA_OPTIONS=" -server -showversion -Xmx512m"
  ;;
...
GS_JARS=${EXT_JARS}$CPS${JSHOMEDIR}$CPS${JSHOMEDIR}/lib/JSpaces.jar$CPS${JSHOMEDIR}/lib/jini/jsk-platform.jar$CPS${JSHOMEDIR}/lib/jini/jsk-lib.jar$CPS${JSHOMEDIR}/lib/jini/start.jar$CPS${JSHOMEDIR}/lib/jini/mahalo.jar$CPS${JSHOMEDIR}/lib/jini/reggie.jar$CPS${JSHOMEDIR}/lib/ServiceGrid/gs-lib.jar$CPS${JSHOMEDIR}/lib/ServiceGrid/gs-boot.jar$CPS${JSHOMEDIR}/lib/common/backport-util-concurrent.jar; export GS_JARS

PLATFORM_VERSION=6.0; export PLATFORM_VERSION
POLICY=${JSHOMEDIR}/policy/policy.all; export POLICY

if [ "$LOOKUPGROUPS" = "" ]; then
LOOKUPGROUPS="gigaspaces-XAP6.0"; export LOOKUPGROUPS
fi
...
if [ "${RMI_OPTIONS}" = "" ] ; then 
  RMI_OPTIONS="-Dsun.rmi.dgc.client.gcInterval=36000000 -Djava.rmi.server.useCodebaseOnly=false -Dsun.rmi.dgc.server.gcInterval=36000000 -Djava.rmi.server.hostname=${NIC_ADDR} -Djava.rmi.server.RMIClassLoaderSpi=default -Djava.rmi.server.logCalls=false"
fi
export RMI_OPTIONS

# For IDE remote debugging add the ${IDE_REMOTE_DEBUG} variable to the command line:
IDE_REMOTE_DEBUG="-Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=y"; export IDE_REMOTE_DEBUG
