...
for i in $( ls -1 /mnt/tarzan/builds/xap/xap10/10.1.0/ | sed -r 's/build_([0-9]+)-([0-9]+)/\1\2 \0/' | sort -r -s -n -k 1,1| sed -r 's/^([0-9])+ (.*)$/\2/' ); do
  ...
  then
      unzip -q -c /mnt/tarzan/builds/xap/xap10/10.1.0/$i/jars/1.5/JSpacesTestSuite.jar META-INF/MANIFEST.MF | grep -Eq "tgrid-suite-target-jvm:\ *(5_Sun8|1_Sun16|6_Sun8|7_Sun8)"
      #unzip -q -c /mnt/tarzan/builds/xap/xap10/10.1.0/$i/jars/1.5/JSpacesTestSuite.jar META-INF/MANIFEST.MF | grep -Eq "tgrid-suite-target-jvm:\ *5_Sun8"
      if [ $? -eq 0 ] 
      then
